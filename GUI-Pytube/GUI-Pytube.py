...
start_layout = [[sg.Input('', key='-INPUT-'), sg.Button('Submit', key='-SUBMIT-')]]
info_tab = [
    ...
    [
        sg.Text('Description:'),
        sg.Multiline('', key='-DESCRIPTION-', size=(40,20), no_scrollbar=False, disabled=True)
    ]
]
download_tab = [
...
window = sg.Window('GUI-Pytube', start_layout)
video_link = ''
...
while True:
    ...
    if event == '-SUBMIT-':
        ...
        # video info
        window = sg.Window('GUI-Pytube', layout, finalize=True)
