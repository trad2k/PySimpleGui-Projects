...
def get_weather_data(location):
    url = f"https://www.google.com/search?hl=en&q=weather+{location.replace(' ','')}"
...
window = sg.Window('GUI-Weather', layout, finalize=True)
...
while True:
    ...
    if event == 'Enter' or event == '<Return>':
        ...
        if weather in ('Rain', 'Chance of Rain', 'Light Rain', 'Showers', 'Scattered Showers', 'Rain and Snow', 'Hail'):
            window['-IMAGE-'].update('rain.png')

        # thunder
        if weather in ('Scattered Thunderstorms', 'Chance of Storm', 'Storm', 'Thunderstorm', 'Chance of TStorm', 'Light thunderstorms and rain'):
            window['-IMAGE-'].update('thunder.png')

        # foggy
        if weather in ('Mist', 'Dust', 'Fog', 'Smog', 'Haze', 'Flurries'):
            window['-IMAGE-'].update('fog.png')
